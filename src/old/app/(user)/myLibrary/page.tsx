...
export default function MyLibraryPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (storedExportCount) {
        setExportCount(parseInt(storedExportCount, 10) || 0);
      }
    } catch (e) {
      // JSON 解析失败，清除损坏的数据
      localStorage.removeItem('myLibrary');
    ...
    try {
      ...
      if (matchingBooks.length > 1) {
        ...
        const exactMatch = allBooks.findIndex((item: any) => {
          ...
          const titleMatch =
            item.data?.central_idea === book.data?.central_idea;
      ...
      if (deletedCount !== 1) {
        console.error(
          `删除异常：预期删除 1 本，实际删除了 ${deletedCount} 本`,
          {
            bookId: bookIdToDelete,
            beforeCount,
        ...
        window.alert(
          `删除异常：检测到 ${matchingBooks.length} 本相同 ID 的绘本`
        );
        return;
      }
  ...
  const handleEdit = (book: any) => {
    ...
    const payload = {
      ...
      illustration_style:
        book.data.illustration_style || book.data.illustration_style_label,
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      // 筛选选中的书籍
      const booksToExport = books.filter((book: any) =>
      ...
      // 调用导出模块
      const success = await exportBooksToPDF(
        booksToExport,
        (current, total) => {
      ...
      <div className="border-4 border-green-300 rounded-md p-4 shadow-lg mt-8 flex justify-around bg-white mb-10">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="text-lg text-blue-500">
            {books.reduce(
              (acc: number, book: any) => acc + book.data.scenes.length,
              0
            )}
          </div>
